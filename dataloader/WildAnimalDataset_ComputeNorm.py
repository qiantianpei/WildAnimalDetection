...
class WildAnimalDataset(Dataset):
    ...
    def __init__(self, csv_file, oversample_list, oversample_times, root_dir, transform, prob, merged_categories = False, norm = 2):
        """
        Args:
    ...
    def read_mean_images(self):
        self.mean_images = {}
        for image in os.listdir('composite_images'):
            self.mean_images[image] = np.array(Image.open('composite_images/' + image)) / 255
    ...
    def __getitem__(self, idx):
        ...
        time = self.label.Light[idx].astype('float')
        camera = self.label.CameraPath[idx]
        ...
        if self.norm == 2: 
            ...
            image_transformed = np.array(transforms.Resize((224,224))(image)) / 255 - self.mean_images[key]
        ...
        sample = [image_transformed, torch.from_numpy(np.array([int(label)])), torch.from_numpy(np.array([time])), camera]
